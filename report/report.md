...
符号位相同且“是否为零”相同才返回 1。`x ^ y` 的最高位直接表示符号是否不同，右移 31 位后取反即可判断符号；对 `!x`、`!y` 异或并取反，区分零与非零。这样 (0,0) 为真，而 (0,1)、(0,-1) 均为假。把两个符号右移结果改成一次异或后再移位，将操作数从 9 降到 8。
...
姓名和学号已填入本报告。提交前仍需在 OBE 填写本人 fork 仓库地址，并完成课程答辩。目前未执行 Git 提交、远端推送或 OBE 提交。
